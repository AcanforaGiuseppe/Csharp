...
namespace LinqTutorial.MethodSyntax
{
    ...
    static class Aggregate
    {
        ...
        public static void Run()
        {
            ...
            // Aggregate performs an operation on each element of the list taking into account the operations that have been performed before

            // Let's use Aggregate to calculate a sum of elements in a collection (we could use Sum method, of course)
            ...
            // Let's use Aggregate to merge letters with a separator (we could use string.Join for that, of course)
            var chars = new[] { "a", "b", "c", "d" };
            var withSeparator = chars.Aggregate((a, b) => a + ',' + b);
            Console.WriteLine($"withSeparator: {withSeparator}");

            // We can use non-default initial value (so-called "seed") we can also implement Count using Aggregate
