...
namespace LinqTutorial.MethodSyntax
{
    static class DotNet6Improvements
    {
        public static void Run()
        {
            ...
            // Before .NET 6 if we wanted to, for example, find the heaviest Pet, we had to do this
            ...
            // Let's say we have a collection of People, and people's Id is identifying them. Unfortunately, our data quality is low, and we have some duplicates.
            // With DistinctBy, we can select people without entries duplicated by Id.
            // The second and next entries with duplicated Id will be discarded
            ...
            // Range operator
            // Before .NET 6 if we wanted, for example, to take elements between index 2 and 6 from a collection, we needed to do this:
            ...
            // We can skip the beginning or the end of the range, for example, to take all alfter index 6:
            ...
            // We can also use it with the index from end operator, for example, to take all elements between index 2 and the 3rd index from the end
            ...
            // If it doesn't require enumeration => if it would, the count will not be returned

            // Regular Count will enumerate collection, and if this collection comes, for example, from a database, it can take long time
