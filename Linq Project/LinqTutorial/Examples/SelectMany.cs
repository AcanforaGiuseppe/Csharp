...
namespace LinqTutorial.MethodSyntax
{
    ...
    static class SelectMany
    {
        ...
        public static void Run()
        {
            ...
            // Let's select all Pets belonging to people, whose names start with letter "J"
            ...
            // Let's go back to the People and Pets example before we selected all Pets belonging to all People
            // but in the end, we ignored People's information let's now select pairs of People and their Pets
        ...
        public static class QuerySyntax
        {
            public static void Run()
            {
                ...
                var allNumbersFromNestedList = from list in nestedListOfNumbers
                                               from number in list
                                               select number;
                Console.WriteLine($"Count of elements in the {nameof(allNumbersFromNestedList)}: {allNumbersFromNestedList.Count()}");
                Printer.Print(allNumbersFromNestedList, nameof(allNumbersFromNestedList));
                ...
                Console.WriteLine($"Count of elements in the {nameof(allNumbersFromVeryNestedList)}: {allNumbersFromVeryNestedList.Count()}");
                Printer.Print(allNumbersFromVeryNestedList, nameof(allNumbersFromVeryNestedList));
            }
        }
