...
namespace LinqTutorial.MethodSyntax
{
    static class SingleElement
    {
        ...
        public static void Run()
        {
            ...
            // We can skip the predicate if we want to access the only element in the collection, this simply changes an one-element collection to a single value
            var numbers2 = new[] { 6 };
            ...
            // The below will thrown an exception because there is no such element in this collection
            //var number99 = numbers.Single(n => n == 99);
            //Printer.Print(number99, nameof(number99));

            // The below will thrown an exception because there are more than one even numbers in this collection
            //var singleEvenNumber = numbers.Single(n => n % 2 == 0);
            //Printer.Print(singleEvenNumber, nameof(singleEvenNumber));

            // SingleOrDefault
            // We can use SingleOrDefault to avoid the exception when there is no such element in the collection
